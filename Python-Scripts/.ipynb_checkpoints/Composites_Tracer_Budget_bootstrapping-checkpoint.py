...
fac = [4.09*1.e6, 12./1.e3] # multiplier to have heat budget in watts and DIC budget in kg/s
depth_levels = [50., 100., 200., 300., 500., 1000.] #, 10000.] 
...
for depth in depth_levels:
    ...
    for var1 in var_list:
        ...
        # Get all budget terms in upper ocean    
        if(var1 == 'thetao'):
            flux_var = 'hfds'
        elif(var1 == 'dissic'):
            flux_var = 'fgco2'
        ...
        #save_var_list = [var1, var1 + '_div_x', var1 + '_div_y', var1 + '_div_z',
        #                 var1 + '_u_div_x', var1 + '_u_div_y', var1 + '_u_div_z', flux_var]
        save_var_list = [var1, var1 + '_u_div_x', var1 + '_u_div_y', var1 + '_u_div_z', flux_var,
        ...
        for region in region_list:
            ...
            if(var1 == 'dissic'):
                ...
                ds1['phyc' + region] = (ds1['phyc' + region]* d1['dz']).sel(lev=slice(depth_lev[0],depth_lev[1])).sum('lev') * fac[k]
                ds1['dt_phyc' + region] = Data_time_tendency(ds1['phyc' + region], ds1['time'], win_month)
                ds1['phyc' + region] = Moving_Avg(ds1['phyc' + region], time = ds1['time'], time_len = win_month)
                
                caco3_flux = - ds1['epcalc100' + region] * fac[k]
                ds1['calcite_flux' + region] = Moving_Avg(caco3_flux, time = ds1['time'], time_len = win_month)
            ...
            ds1[var1 + '_res' + region] = (ds1['dt_' + var1 + region] - ds1[var1 + '_ver_div' + region] 
                                           - ds1[var1 + '_hor_div' + region] - ds1[flux_var + region]) # residual term
            ds1[var1 + '_vert_flx_conv' + region] = (ds1[var1 + '_res' + region] + 
                                                     ds1[flux_var + region])
