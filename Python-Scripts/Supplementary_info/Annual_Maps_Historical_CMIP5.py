"""
This script can be used to composites of anomalies in surface temperatures, salinity, DIC and nutrients
in response to a single North Atlantic Oscillation (NAO) in cmip5 historical runs.

In this script, composites are created by combining NAO+ and NAO- phases by simply multypling anomalies in NAO- by -1.
"""
...
for exp in experiment_id:
    for model in source_id:
        ...
        for dir1 in dir_list:
            ...
            ds_NAO = xr.open_dataset(save_path + model + exp + "/" + dir_name + "/Timeseries/NAO_SLP.nc", 
            ...
            for var1 in var_list:
                ...
                if(var1 == 'tos'):
                    file_path = (cmip_dir + model + exp + "/mon/ocean/Omon/" + 
                                 dir_name + "/latest/" + var1)
                elif(var1 == 'dissic'):
                    file_path = (cmip_dir + model + exp + "/mon/ocnBgchem/Omon/" + 
                                 dir_name + "/latest/" + var1)

                d1 = xr.open_mfdataset(file_path + "/" + var1 + "*nc", chunks={'time':-1}, use_cftime=True)
                ...
                d1 = d1.isel(y=slice(int(len(d1['y'])/2), len(d1['y']))) # only North Atlantic surface data

                ds = xr.merge([d1.get([var1]), 
                ...
                for cas in case:
                    ...
                    for year in range(year_str + int(num_year/2), len(nao_DJF) - year_end - int(num_year/2)):
                        ...
                        if(count_NAO.isel(time=year) == 1):
                            ...
                            tmp = annual_mean_data(tmp, [var1], tmp['time'], method = 'mean')
                ...
                directory = save_path + model + exp + "/" + dir_name + "/Annual_Maps"
                if not os.path.exists(directory):
                    # If it doesn't exist, create it
                    os.makedirs(directory)
                    
                save_file_path = (save_path + model + exp + "/" + dir_name + "/Annual_Maps/" + var1 + ".nc")
